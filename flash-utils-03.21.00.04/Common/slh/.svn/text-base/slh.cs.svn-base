...
namespace TIBootAndFlash
{
  ...
  public enum MagicFlags : uint
  {
    ...
    SLT_MAGIC_LOADIMAGE_IRAM = 0x53465400,  // Download a UBL-like image to execute directly in IRAM
    SLT_MAGIC_LOADIMAGE_DDR = 0x534654EE	  // Download SLT and then load image to DDR via it
  };
  ...
  partial class Program
  {
    ...
    private static void DispHelp()
    {
      ...
      Console.WriteLine("\t\t-load2IRAM  \tLoad a small UBL-like image to IRAM.");
      Console.WriteLine("\t\t-load2DDR   \tLoad a larger image to DDR.\n");
      if (deviceTypes.Length > 1)
      {
      ...
      Console.WriteLine("\t\t-startAddr <Application entry point address>\tSpecify in hex");
      Console.WriteLine("\t\t-loadAddr  <Application image load address> \tSpecify in hex (only valid with load2DDR mode)");
    }   
    ...
    public static void WorkerThreadStart()
    {
      switch (cmdParams.CMDMagicFlag)
      {
        ...
        case MagicFlags.SLT_MAGIC_LOADIMAGE_DDR:
        {
          ...
          //Console.WriteLine("Waiting for DONE that indicates process complete.");
    ...
    private static void TransmitSLTAndApp()
    {
      ...
      EmbeddedFileIO.ExtractFile(System.Reflection.Assembly.GetExecutingAssembly(), "slt_"+cmdParams.deviceType+".bin", true);
      
      // Read the extracted embedded SLT data that we will transmit
      imageData = FileIO.GetFileData("slt_" + cmdParams.deviceType + ".bin");
      ...
      BOOTUBLSEQ1:
        ...
        Console.WriteLine("\nWaiting for SLT on the "+cmdParams.deviceType+"...");
