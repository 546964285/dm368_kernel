...
namespace TIBootAndFlash
{
...
#if DM35X_STANDARD
    MAGIC_NUMBER_VALID          = 0xA1BCED00,
    UBL_MAGIC_SAFE              = 0xA1BCED00,
    UBL_MAGIC_DMA               = 0xA1BCED11,	            /* DMA boot mode */
    UBL_MAGIC_IC                = 0xA1BCED22,	            /* I Cache boot mode */
    UBL_MAGIC_FAST              = 0xA1BCED33,	            /* Fast EMIF boot mode */
    UBL_MAGIC_DMA_IC            = 0xA1BCED44,	            /* DMA + ICache boot mode */
    UBL_MAGIC_DMA_IC_FAST       = 0xA1BCED55,	            /* DMA + ICache + Fast EMIF boot mode */
#else
  ...
  partial class Program
  {
    ...
    private static void DispHelp()
    {
      ...
      Console.WriteLine("\t\t-flash      \tPlace a secondary user boot loader (UBL) and applicaion image in the");
      ...
      Console.WriteLine("\t\t-appStartAddr <Application entry point address>\tSpecify in hex (defaults to 0x81080000)");
      Console.WriteLine("\t\t-appLoadAddr  <Application image load address> \tSpecify in hex (defaults to 0x81080000)");      
      Console.WriteLine("\t\t-ublStartAddr <UBL entry point address>        \tSpecify in hex (defaults to 0x00000100)");
    }   
    ...
    public static ProgramCmdParams ParseCmdLine(String[] args)
    {
        ...
        if (s.StartsWith("-"))
        {
          switch (s.Substring(1).ToLower())
          {
            case "flash":
              ...
              numFiles = 2;
              cmdString = "Flashing UBL and application image: ";
              break;
            case "flash_noubl":
              if (myCmdParams.CMDMagicFlag == MagicFlags.MAGIC_NUMBER_INVALID)
              {
              ...
              cmdString = "Flashing single boot image: ";
              numFiles = 1;
              break;
            case "erase":
              if (myCmdParams.CMDMagicFlag == MagicFlags.MAGIC_NUMBER_INVALID)
          ...
          if ((s.StartsWith("-")) && (argsHandled[i] != true))
          {
            switch (s.Substring(1).ToLower())
            {
              case "targettype":
                ...
                if (index < 0)
                {
                  Console.WriteLine("Target type not recongnized, using default ({0}).",myCmdParams.deviceType);
                }
      ...
      // Prep command string 
      cmdString = cmdString + " the " + myCmdParams.flashType + " flash device on the " + myCmdParams.deviceType;
    ...
    public static void WorkerThreadStart()
    {
      ...
      BOOTUBLSEQ1:
        ...
        if (!status)
        {
          Console.Write("Command failed, retrying\n");
          goto BOOTUBLSEQ1;
        }
      }
      catch (Exception e)
