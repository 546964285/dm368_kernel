...
Uint32 DEVICE_init()
{
  ...
  switch (deviceOPP ) {  
    case DEVICE_OPP_1P3V_456MHZ:
      ...
      if (status == E_PASS) status |= DEVICE_PLL0Init ( 0, 18, 0, 0, 0, 4, 5 );
        break;
    ...
    case DEVICE_OPP_1P3V_408MHZ:      
      ...
      if (status == E_PASS) status |= DEVICE_PLL0Init(0, 16, 0, 0, 0, 4, 5);   
      break;
    case DEVICE_OPP_1P2V_372MHZ:
      ...
      if (status == E_PASS) status |= DEVICE_PLL0Init(0, 30, 1, 0, 0, 3, 5);
        break;
    case DEVICE_OPP_1P2V_300MHZ:
      ...
      if (status == E_PASS) status |= DEVICE_PLL0Init (0, 24, 0, 1, 0, 11, 5);
        break;
    default :
  ...
  if (status == E_PASS)
  {
    ...
    if(status == E_PASS) status |= DEVICE_SDMMCInit(0); 
  #endif
  #endif
  ...
  if (status == E_PASS)
  {
      ...
      status |= DEVICE_ExternalMemInit(0x000000C4, 0x0A034622, 0x184929CB, 0x380FC700, 0x00000407, 0x00000000);
	#endif
  }
#endif
...
#elif UBL_NOR
Uint32 DEVICE_NORInit( void )
{
  /* Configure GP0[11] as gpio */
  DEVICE_pinmuxControl(0, 0x000F0000, 0x00080000);
  /* Configure GP0[11] as output */
  *GPIO0_DIR &= 0xFFFFF7FF;
  /* Drive GP0[11] low */
  *GPIO0_CLR |= 0x800;
...
Uint32 DEVICE_UARTInit(Uint8 periphNum)
{
  ...
  i = (gDeviceOpPoint.megaHz * 1000000 ) / 2;
  mul = DEVICE_UART0_DESIRED_BAUD*DEVICE_UART0_OVERSAMPLE_CNT;
  while ( ( divisor++) * mul < i );
